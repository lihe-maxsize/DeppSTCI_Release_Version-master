...
def data_generate_func(intervention_time=[], seed=1, max_time=10, intervention_C=3, n=10, m=10, with_lambda=False):
    """
    ...
    SY_pre = SY0
    SW_pre4 = [SW0]
    lambda_max = 300  # lambda_max should be greater than all lambda_w and lambda_y
    ...
    while (time_step <= max_time):
        ...
        X3_relative_points = get_relative_points(rou0_x3, rou1_x3)
        X4_relative_points = get_relative_points(rou0_x4, rou1_x4)
        ...
        # observation w value
            ...
            # confounders
            X_1w = get_X1_array(w_x, w_y)
            X_2w = get_X2_array(w_x, w_y)
            ...
            lambda_w = lambda_w_array(alpha0, alphax, [X_1w, X_2w, X_3w, X_4w], alphaw, SW_pre, alphay, SY_pre, w_x,
                                      w_y)
            # reject sampling
        ...
        if time_step + 1 in intervention_time:
            ...
            X_1w_next = get_X1_array(wx_next, wy_next)
            X_2w_next = get_X2_array(wx_next, wy_next)
            ...
            if with_lambda:
                ...
                lambda_X_1w_next = get_X1_array(lambda_x, lambda_y)
                lambda_X_2w_next = get_X2_array(lambda_x, lambda_y)
                ...
                lambda_next = lambda_w_array(alpha0, alphax,
                                             [lambda_X_1w_next, lambda_X_2w_next, lambda_X_3w_next, lambda_X_4w_next],
                                             alphaw, SW_pre, alphay, SY_pre, lambda_x, lambda_y)
            ...
            lambda_w_next = lambda_w_array(alpha0, alphax, [X_1w_next, X_2w_next, X_3w_next, X_4w_next], alphaw, SW_pre,
                                           alphay, SY_pre, wx_next, wy_next)
        ...
        X_1o = get_X1_array(o_x, o_y)
        X_2o = get_X2_array(o_x, o_y)
        ...
        if len(SW_pre4) > 4:
            SW_pre4.pop(0)
        lambda_o = lambda_o_array(gama0, gamax, [X_1o, X_2o, X_3o, X_4o], gama2, X_2o, gamaw, SW_pre4, gamay, SY_pre,
                                  o_x, o_y)
...
class Data_generator(object):
    def __init__(self, T, N, C, M, base):
        """
        :param T: total time
    ...
    def generating_observation(self):
        ...
        if os.path.exists(self.base + raw_data_dict_path) and os.path.exists(self.base + raw_data_list_path):
            ...
            for seed in SEEDS:
                ...
                raw_data, _ = data_generate_func(intervention_time=[], seed=seed, max_time=self.T,
        ...
        if os.path.exists(self.base + intervention_data_dict_path):
            ...
            for seed in SEEDS:
                for it in range(2, self.T - self.M):
                    ...
                    intervention_data, _ = data_generate_func(intervention_time=intervention_time,
        ...
        if os.path.exists(self.base + intervention_data_dict_path):
            ...
            for seed in SEEDS:
                for it in range(2, self.T - self.M + 2):
                    intervention_time = list(range(it, it + self.M))
                    intervention_data, lambda_res = data_generate_func(intervention_time=intervention_time,
